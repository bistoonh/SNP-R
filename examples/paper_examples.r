...
#' @examples
#' \dontrun{
#' source("examples/paper_examples.R")
#' example_stepwise()
#' }
example_stepwise <- function() {
  ...
  mse_diff <- mean((snp_result$y_k_opt - dgcv_result$y_h_opt)^2)
  
  cat(sprintf("\nPerformance Summary:\n"))
  cat(sprintf("SNP time:    %.4f seconds\n", snp_result$time_elapsed))
  cat(sprintf("DGCV time:   %.4f seconds\n", dgcv_result$time_elapsed))
  cat(sprintf("Speedup:     %.2fx\n", speedup))
  cat(sprintf("MSE diff:    %.6f\n", mse_diff))
  ...
  return(list(
    ...
    data = list(x = x, y = y, y_true = y_true),
    performance = list(speedup = speedup, mse_diff = mse_diff)
  ))
}
...
example_wavy <- function() {
  ...
  cat("\nRunning DGCV...\n")
  dgcv_result <- DGCV(x, y, num_h_points = 25)
...
run_all_examples <- function() {
  ...
  if (!is.null(result1) && !is.null(result2) && !is.null(result3)) {
    cat("\nPerformance Summary:\n")
    cat(sprintf("%-15s %10s %10s %8s %10s\n", "Example", "SNP Time", "DGCV Time", "Speedup", "MSE Diff"))
    cat(paste(rep("-", 55), collapse=""), "\n")
    cat(sprintf("%-15s %10.4f %10.4f %8.2fx %10.2e\n", 
                "Stepwise", result1$snp_result$time_elapsed, result1$dgcv_result$time_elapsed, 
                result1$performance$speedup, result1$performance$mse_diff))
    cat(sprintf("%-15s %10.4f %10.4f %8.2fx %10.2e\n", 
                "Wavy", result2$snp_result$time_elapsed, result2$dgcv_result$time_elapsed, 
                result2$performance$speedup, result2$performance$mse_diff))
    cat(sprintf("%-15s %10.4f %10.4f %8.2fx %10.2e\n", 
                "Housing", result3$snp_result$time_elapsed, result3$dgcv_result$time_elapsed, 
                result3$performance$speedup, result3$performance$mse_diff))
  }
