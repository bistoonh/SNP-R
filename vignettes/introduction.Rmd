...
## Customizing SNP Parameters

SNP allows fine-tuning through two key parameters:
...
# Compare results
plot(x, y, pch=16, col="gray", main="SNP Parameter Comparison")
lines(x, snp_result$y_k_opt, col="red", lwd=2)
lines(x, snp_fast$y_k_opt, col="orange", lwd=2, lty=2)
lines(x, snp_thorough$y_k_opt, col="darkred", lwd=2, lty=3)
legend("topright", 
...
# Performance comparison
cat("Default time:", snp_result$time_elapsed, "seconds\n")
cat("Fast time:", snp_fast$time_elapsed, "seconds\n") 
cat("Thorough time:", snp_thorough$time_elapsed, "seconds\n")
```
...
- **For faster computation**: Reduce `num_h_points` (20-30) and `num_slices` (30-40)
- **For higher accuracy**: Increase `num_h_points` (50-80) and `num_slices` (80-120)
- **Default values**: Provide good balance between speed and accuracy for most applications
- **Large datasets (n > 5000)**: Consider reducing parameters to maintain reasonable runtime
